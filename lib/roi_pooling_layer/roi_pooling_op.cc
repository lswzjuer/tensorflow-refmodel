 ...
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    for (int n = 0; n < num_rois; ++n) 
    {
      ...
      T bin_size_h_1 = static_cast<T>(roi_height) * static_cast<T>(10923);
      T bin_size_w_1 = static_cast<T>(roi_width)  * static_cast<T>(10923);

	  //const T bin_size_h = static_cast<T>(static_cast<int>(bin_size_h_1*static_cast<T>(2<<8)))/static_cast<T>(2<<8);
	  //const T bin_size_w = static_cast<T>(static_cast<int>(bin_size_w_1*static_cast<T>(2<<8)))/static_cast<T>(2<<8);	
	  const T bin_size_h = static_cast<T>(round(bin_size_h_1/static_cast<T>(2<<4))*static_cast<T>(2<<4));
	  const T bin_size_w = static_cast<T>(round(bin_size_w_1/static_cast<T>(2<<4))*static_cast<T>(2<<4));	

	  // Commented for hardware emulation. Abinash 
      // TODO: Make this selection using an input flag
      ...
      for (int ph = 0; ph < pooled_height_; ++ph) 
      {
        for (int pw = 0; pw < pooled_width_; ++pw) 
        {
          ...
          int hstart = static_cast<int>(floor(static_cast<T>(ph)
                                              * bin_size_h/static_cast<T>(2<<15)));
          int wstart = static_cast<int>(floor(static_cast<T>(pw)
                                              * bin_size_w/static_cast<T>(2<<15)));
