...
# @Last Modified time: 2019-05-16 16:05:11
import numpy as np 
...
def clip_graph_2(model_path):
    ...
    with tf.Graph().as_default() as graph:
        with tf.Session(graph=graph) as sess:
            ...
            new_input1=tf.placeholder(dtype=tf.float32,name="pool5_Conv_Relu")

            new_input2=tf.placeholder(dtype=tf.float32,name="pool4_Conv_Relu")

            new_input3=tf.placeholder(dtype=tf.float32,name="pool3_Conv_Relu")

            graph1=load_graph(model_path)
            z,=tf.import_graph_def(graph1,input_map={"pool5_Conv/Relu:0":new_input1,"pool4_Conv/Relu:0":new_input2,"pool3_Conv/Relu:0":new_input3},
                                        return_elements=["ground_truth/Reshape:0"])
            output=tf.identity(z,"new_ground_truth/Reshape")
            ...
            # 训练完成之后保存模型为.pb文件
            with tf.gfile.FastGFile('./three_clip_model.pb',mode='wb') as f:
                f.write(const_graph.SerializeToString())    
...
def load_and_read_graph(model_path,new_model_path):
    ...
    with tf.Graph().as_default() as graph:
        ...
        image=tf.placeholder(dtype=tf.float32,shape=[None,1088,1920,3],name='image')
        graph_def1=load_graph(model_path)

        tf.import_graph_def(graph_def1,input_map={'image:0':image},name='')
        skip_sum0=tf.get_default_graph().get_tensor_by_name('skip_layer_sum_0_0/add:0')
        print(skip_sum0)
        skip_sum1=tf.get_default_graph().get_tensor_by_name('skip_layer_sum_0_1/add:0')
        print(skip_sum1)
        true_div=tf.get_default_graph().get_tensor_by_name('act/truediv:0')
        print(true_div)
        output_put1=tf.get_default_graph().get_tensor_by_name('ground_truth/Reshape:0')
        print(output_put1)

        graph_def2=load_graph(new_model_path)
        tf.import_graph_def(graph_def2,input_map={'new_skip_layer_sum_0_1:0':skip_sum1},name='')
        
        output_put2=tf.get_default_graph().get_tensor_by_name('new_ground_truth/Reshape:0')
        my_name=tf.identity(output_put2,name="my_name")
        print(output_put2)

        with tf.Session(graph=graph) as sess:

            results2=sess.run(my_name,feed_dict={image:test_data})
            print(results2)
...
if __name__=="__main__":
    ...
    model_path=r"./mscoco_fcn_padded_lscape_prediction_tf_graph.pb"
