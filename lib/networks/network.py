...
class Network(object):
        ...
        # Todo
        def fp(self,input,fl_y):
            return fp_op.fp(input,fl_y)

        @layer
        def inverse_quantization(self,input,name,fl_y):

            with tf.variable_scope(name) as scope:
                inver_value=self.fp(input,fl_y)
                print(inver_value)
                return inver_value
        @layer
        def Reshape(self,input,name):
            input_shape=tf.shape(input)
            output_shape=[-1,120,80,3]
                ...
                with tf.variable_scope(name) as scope:
                        ...
                        if biased:
                                ...
                                if relu:
                                        ...
                                        if self.isHardware:
                                                bias_s = self.saturate(bias, cfg.WORD_WIDTH)
                                ...
                                if self.isHardware:
                                        s_biasadd=self.saturate(bias_add, cfg.WORD_WIDTH)  # New addition for saturation
                                        print(s_biasadd)
                                        return s_biasadd
                                print(bias_add)
                                return bias_add
        ...
        def deconv(self, input, k_h,k_w, c_o,s_h,s_w,name, bw=cfg.WORD_WIDTH, fl_w=0,fl=3,fl_y=3, rs=0,rate=1,
                ...
                with tf.variable_scope(name) as scope:
                        ...
                        if biased:
                                ...
                                if relu:
                                        relu_conv=tf.nn.relu(deconv)
                                        print(relu_conv)
                                        return relu_conv
                                print(deconv)
                                return deconv
        @layer
        def ResizeBilinear(self,input,name):
            shape=tf.shape(input)
            new_h=shape[1]*2
            new_w=shape[2]*2
            output_size=[new_h,new_w]
            return tf.image.resize_bilinear(input,output_size,align_corners=True,name=name)
        
        @layer
        def Sub(self,input,name):
            return tf.subtract(input[0],input[1],name=name)

        @layer
        def RealDiv(self,input,name):
            return tf.truediv(input[0],input[1],name=name)

        @layer
        def Sum(self,input,name,reduction_indices=-1, keep_dims=True):
            return tf.reduce_sum(input,reduction_indices=-1,keep_dims=True,name=name)

        # @layer
        # def Mul(self,input,name):
        #     return tf.reduce_max(input,reduction_indices=-1,keep_dims=True,name=name)

        @layer
        def Exp(self,input,name):
            return tf.exp(input,name=name)

        @layer
        def Max(self,input,name, reduction_indices=-1, keep_dims=True):
            return tf.reduce_max(input,reduction_indices=-1,keep_dims=True,name=name)
