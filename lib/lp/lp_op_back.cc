 ...
 public:
  explicit LpOp(OpKernelConstruction* context) : OpKernel(context) {
    ...
    OP_REQUIRES_OK(context,
                   context->GetAttr("bw", &bw_));
    OP_REQUIRES(context, bw_>= 0,
                errors::InvalidArgument("Need bw >= 0, got ",
                                        bw_));
    // Get the Fractional Length
    OP_REQUIRES_OK(context,
                   context->GetAttr("fl", &fl_));
    OP_REQUIRES(context, fl_ <= bw_,
                errors::InvalidArgument("Need fl <= bw, got ",
                                        fl_));
  }
