...
%Delay_ms_CONVs = 10^3*Cycle_CONVs./(Clk_Freq_MHz*10^6); % ms
...
MAC_utilization = sum(NUM_MAC_origin)/sum(NUM_MAC_actual);
...
DDR3_Freq_MHz  = 500*0.7; % 1300 MHz % MAX = 266*8 = 2120; 44/64 = 0.6875 
...
ms_overlap_pos_CONV = zeros(NUM_CONV,1);
ms_overlap_pre_CONV_L1   = Delay_ms_pTil_RDpx_CONV(1)+Delay_ms_pTil_RDwt_CONV(1);
ms_overlap_pos_CONV(end) = Delay_ms_pTil_WRpx_CONV(end);
for L = 1 : NUM_CONV-1
    ms_overlap_pos_CONV(L) = max(Delay_ms_pTil_WRpx_CONV(L), Delay_ms_pTil_RDpx_CONV(L+1)+Delay_ms_pTil_RDwt_CONV(L+1));
end
ms_pExt_CONV = sum(ms_overlap_pos_CONV) + ms_overlap_pre_CONV_L1;
...
Delay_ms_total = Delay_ms_CONV + Delay_ms_PLMX + Delay_ms_PLAV + Delay_ms_EWIS + Delay_ms_FCON;
