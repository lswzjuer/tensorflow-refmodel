...
CR_CONV_inpx_cmd_size = NIX_CONV.*Tiy_CONV_DMA*(WD_PX/8)*FCT_DMA; % 32-bit, csr_inpx_cmd_size, csr_W_LEN_RDWT_CV
...
len_WRpx_pTil_Bytes_CONV_raw = Tox_CONV.*Toy_CONV0*(WD_PX/8)*FCT_DMA;
...
% transfer length (bytes) per transaction (descriptor) 
CR_PLMX_inpx_cmd_size = Tix_PLMX.*Tiy_PLMX*(WD_PX/8)*FCT_DMA; % 24-bit, csr_W_LEN_RDPX_PL
...
% transfer length (bytes) per transaction (descriptor) 
CR_PLMX_outpx_cmd_size = Tox_PLMX.*Toy_PLMX_WRpx*(WD_PX/8)*FCT_DMA; % 24-bit, csr_W_LEN_WRPX_PL
...
% transfer length (bytes) per transaction (descriptor) 
CR_EWIS_inpx_cmd_size = NIX_EWIS.*Tiy_EWIS*(WD_PX/8)*FCT_DMA;
